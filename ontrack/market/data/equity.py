...
class PullEquityData:
    ...
    def __parse_equity_eod_data(self, record):
        ...
        entity["open_price"] = nh.str_to_float(record["OPEN_PRICE"])
        entity["high_price"] = nh.str_to_float(record["HIGH_PRICE"])
        entity["low_price"] = nh.str_to_float(record["LOW_PRICE"])
        entity["last_price"] = nh.str_to_float(record["LAST_PRICE"])
        entity["close_price"] = close_price
        entity["avg_price"] = nh.str_to_float(record["AVG_PRICE"])
        entity["point_changed"] = price_change
        entity["percentage_changed"] = percentage_change
        entity["traded_quantity"] = traded_quantity
        entity["traded_value"] = nh.str_to_float(record["TURNOVER_LACS"])
        entity["number_of_trades"] = number_of_trades
        entity["quantity_per_trade"] = quantity_per_trade
        entity["delivery_quantity"] = nh.str_to_float(record["DELIV_QTY"])
        entity["delivery_percentage"] = nh.str_to_float(record["DELIV_PER"])
        entity["date"] = date
