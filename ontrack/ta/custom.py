...
def import_dir(path, verbose=True):
    ...
    for d in dirs:
        ...
        # only look in directories which are valid ontrack.ta categories
        if dirname not in [*ontrack.ta.Category]:
            if verbose:
                print(
                    f"[i] Skipping the sub-directory '{dirname}' since it's not a valid ontrack.ta category."
                )
            continue
        ...
        for module in glob(abspath(join(path, dirname, "*.py"))):
            ...
            # figure out which of the modules functions to bind to ontrack.ta
            ...
            if module_name not in ontrack.ta.Category[dirname]:
                ontrack.ta.Category[dirname].append(module_name)
...
import_dir.__doc__ = """
Import a directory of custom indicators into ontrack.ta
...
together with the existing ontrack.ta functions just like if they were part of
ontrack.ta.

If you at some late point would like to push them into the ontrack.ta library
...
indicators. Invoke ontrack.ta.custom.import_dir once to pre-populate it with
sub-folders for all available indicator categories, e.g.:

>>> import os
>>> from os.path import abspath, join, expanduser
>>> from ontrack.ta.custom import create_dir, import_dir
>>> ta_dir = abspath(join(expanduser("~"), "my_indicators"))
>>> create_dir(ta_dir)

3. You can now create your own custom indicator e.g. by copying existing
ones from ontrack.ta core module and modifying them.
...
available in categories under the ontrack.ta-folder. The only difference will
...
like all other native indicators in ontrack.ta, including help functions.
"""
