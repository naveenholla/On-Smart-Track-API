...
from ontrack.ta.overlap import hlc3, ma
from ontrack.ta.utils import get_drift, get_offset, signed_series, verify_series
...
def kvo(
    ...
    **kwargs,
    ...
    sv = signed_volume.loc[
        signed_volume.first_valid_index() :,
    ]
    kvo = ma(mamode, sv, length=fast) - ma(mamode, sv, length=slow)
    kvo_signal = ma(
        mamode,
        kvo.loc[
            kvo.first_valid_index() :,
        ],
        length=signal,
    )
